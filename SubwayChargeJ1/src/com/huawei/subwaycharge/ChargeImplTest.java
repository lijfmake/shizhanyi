...
public class ChargeImplTest {
	...
	public void testOpQuery() {
		...
		//q 10 20:10 22.10
		Assert.assertEquals(chargeImpl.opQuery(10, 20, 10, 22, 10).toString(),OpResult.createOpResult(ReturnCodeEnum.E01).toString()
				);
		...
		Assert.assertEquals(OpResult.createOpResult(ReturnCodeEnum.I20,logs26).toString(),
				chargeImpl.opQuery(0,0,0,23,59).toString()
				);
    	//29  q 3 00:00 23:59
//		Assert.assertEquals(chargeImpl.opQuery(3,0,0,23,59).toString(), 
//				OpResult.createOpResult(ReturnCodeEnum.E21).toString());
		...
		//25  q 0 20:00 23:59
		    	...
		    	//<1><10:30><S3><11:59><S4><2><成功>
		    	//<2><8:30><S3><9:30><S4><2><成功>
		    	ChargeLogInfo logs1=new ChargeLogInfo(1,9,30,"S2",9,59,"S3",2,true);
		    	ChargeLogInfo logs2=new ChargeLogInfo(1,10,30,"S3",11,59,"S4",2,true);
		    	ChargeLogInfo logs3=new ChargeLogInfo(2,8,30,"S3",9,30,"S4",2,true);
		    	ChargeLogInfo[] logs26={logs1,logs2,logs3};    	
				Assert.assertEquals(chargeImpl.opQuery(0,0,0,23,59).toString(), 
						OpResult.createOpResult(ReturnCodeEnum.I20,logs26).toString());
		    	...
		    	ChargeLogInfo logs4=new ChargeLogInfo(1,9,30,"S2",9,59,"S3",2,true);
		    	ChargeLogInfo logs5=new ChargeLogInfo(1,10,30,"S3",11,59,"S4",2,true);  	
		    	ChargeLogInfo[] logs28={logs4,logs5};    	
				Assert.assertEquals(chargeImpl.opQuery(0,0,0,23,59).toString(), 
						OpResult.createOpResult(ReturnCodeEnum.I20,logs28).toString());
	...
	public void testOpCharge() {
		...
		Assert.assertEquals(chargeImpl.opCharge(ci15, distances).toString(), 
				...
				/*		//c 1 B 100 10:30 S2 10:59 S3
		ChargeCmdInfo ci19 = new ChargeCmdInfo(1, CardTypeEnum.B, 100, 10,
				30, "S2", 10, 59, "S3");
		...
		//c 1 B 100 10:30 S2 10:59 S3
		ChargeCmdInfo ci20 = new ChargeCmdInfo(1, CardTypeEnum.B, 100, 10,
				30, "S2", 10, 59, "S3");
		...
		*/
				//c 1 B 100 10:30 S2 10:59 S3
/*				ChargeCmdInfo ci13 = new ChargeCmdInfo(1, CardTypeEnum.B, 100, 10,
						30, "S2", 10, 59, "S3");
				Assert.assertEquals(chargeImpl.opCharge(ci13, distances).toString(), 
						OpResult.createOpResult(ReturnCodeEnum.I11,1,99).toString());*/
				...
				Assert.assertEquals(chargeImpl.opCharge(ci212, distances).toString(), 
						OpResult.createOpResult(ReturnCodeEnum.I11,1,97).toString());
				//C 3 C 85 09:05 S3 10:201 S8
/*				ChargeCmdInfo ci17 = new ChargeCmdInfo(3, CardTypeEnum.C,85, 9,
						05, "S3", 10, 201, "S8");
				Assert.assertEquals(chargeImpl.opCharge(ci17, distances).toString(), 
						OpResult.createOpResult(ReturnCodeEnum.I,3,85).toString());*/
				//c 1 A 100 09:30 S3 09:39 S3
